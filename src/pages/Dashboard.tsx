      ...
      <div className="p-8">
          ...
          {stats.map((stat, index) => (
            <div key={index} className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              ...
              <p className={`text-2xl font-bold ${stat.textColor}`}>
                {typeof stat.value === 'number' && stat.title !== 'Omset Hari Ini' ? stat.value : stat.value}
              </p>
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <h2 className="text-lg font-semibold text-gray-900 mb-4">Produk Stok Menipis</h2>
              ...
              {transactions.slice(0, 5).map(transaction => (
                <div key={transaction.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  <div>
                    ...
                    <p className="text-sm text-gray-600">{transaction.cashier}</p>
                  </div>
